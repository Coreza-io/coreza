...
const BaseNode: React.FC<BaseNodeProps> = ({ data, selected, children }) => {
    ...
    (save: boolean = true) => {
      if (save && editingName.trim() && editingName.trim() !== displayName) {
        ...
        if (existingNames.includes(finalName.toLowerCase())) {
          let counter = 1;
          while (
            existingNames.includes(`${finalName}${counter}`.toLowerCase())
          ) {
            counter++;
          }
  ...
  useEffect(() => {
    ...
    const fieldKeysChanged =
      definition.fields.length !== Object.keys(fieldState).length ||
  ...
  // ========== New batch updater ===========
  const handleFieldStateBatch = useCallback(
  ...
  const getFieldPreview = (fieldKey: string) => {
    ...
    const srcNode =
      previousNodes.find((n) => n.id === srcId) || selectedPrevNode;
  ...
  const handleSubmit = async (e?: React.FormEvent, inputOverride?: any) => {
    ...
    console.log(
      "🎯 [NODE EXECUTION] Starting execution for node:",
    ...
    for (const f of definition?.fields || []) {
      ...
      if (f.displayOptions?.show) {
        for (const [depKey, rawAllowed] of Object.entries(
          f.displayOptions.show
        )) {
          const allowedValues = Array.isArray(rawAllowed)
    ...
    try {
      ...
      if (definition?.name === "Loop") {
        ...
        else if (typeof payload.inputArray === "object")
          loopItems = [payload.inputArray];
        else
          throw new Error(
            "Loop input must be an array or JSON string representing an array"
          );
        ...
        const inputChanged =
          !Array.isArray(prev.loopItems) || (prev.loopSig ?? "") !== loopSig;
        ...
        const edgeArrivals = incomingToLoop
          .map((e) => edgeBuf[e.id])
          .filter((v) => v != null);

        if (loopIndex > maxLoopIndex) {
          finished = true;
          const next = executionStore.advanceLoop(
            nodeId!,
            current,
            loopIndex + 1,
            finished,
      ...
      const operationField = (definition?.fields || []).find(
        (f: any) => f.key === "operation"
      );
      let operationMethod = "POST";

      if (operationField) {
        const opSelected = (operationField.options || []).find(
          (opt: any) => opt.id === fieldState.operation
        );
  ...
  useEffect(() => {
    const handleAutoExecute = async (event: CustomEvent) => {
      if (event.detail?.nodeId === nodeId) {
        ...
        if (event.detail.allEdges && event.detail.allNodes) {
          const incomingEdges = event.detail.allEdges.filter(
            (edge: any) => edge.target === nodeId
          );
          const isConditionalTarget = incomingEdges.some((edge: any) => {
            const sourceNode = event.detail.allNodes.find(
    ...
    window.addEventListener(
      "auto-execute-node",
      handleAutoExecute as EventListener
    );
    return () => {
  ...
  useEffect(() => {
    ...
    if (
      ...
      resultToPass &&
      typeof resultToPass === "object" &&
      "result" in resultToPass
    ) {
