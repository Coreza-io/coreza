...
export class IndicatorService {
  static async calculateRSI(input: IndicatorInput): Promise<IndicatorResult> {
    ...
    return { indicator: 'RSI', period, values: results, latest: results.pop() ?? null };
  }
  ...
  static async calculateSMA(input: IndicatorInput): Promise<IndicatorResult> {
    ...
    return { indicator: 'SMA', period, values: results, latest: results.pop() ?? null };
  }

  static async calculateMACD(input: IndicatorInput): Promise<IndicatorResult> {
    const { prices: rawPrices, candle_data: rawCandles, fastPeriod: rawFast = 12, slowPeriod: rawSlow = 26, signalPeriod: rawSignal = 9 } = input;
    const prices = extractPrices(rawPrices, rawCandles);
    ...
    return { indicator: 'MACD', periods: { fast, slow, signal }, values: results, latest: results.pop() ?? null };
  }

  static async calculateBollinger(input: IndicatorInput): Promise<IndicatorResult> {
    const { prices: rawPrices, candle_data: rawCandles, period: rawPeriod = 20, stdDev: rawStd = 2 } = input;
    const prices = extractPrices(rawPrices, rawCandles);
    ...
    return { indicator: 'BollingerBands', period, stdDev, values: results, latest: results.pop() ?? null };
  }
  ...
  static async calculateADX(input: IndicatorInput): Promise<IndicatorResult> {
    ...
    return { indicator: 'ADX', period, values: adxResult, latest: adxResult.pop() ?? null };
  }
  ...
  static async calculateStochastic(input: IndicatorInput): Promise<IndicatorResult> {
    ...
    return { indicator: 'Stochastic', periods: { k: periodK, d: periodD }, values: stochResult, latest: stochResult.pop() ?? null };
  }

  static async calculateIchimoku(input: IndicatorInput): Promise<IndicatorResult> {
    const { candle_data: rawCandles, conversion_period: rawConv = 9, base_period: rawBase = 26, leading_span_b_period: rawSpan = 52, displacement: rawDisp = 26 } = input;
    const candles = parseArray<any>(rawCandles, 'candle_data');
    ...
    return { indicator: 'IchimokuCloud', periods: { conversion: conv, base, span_b: span, displacement: disp }, values: ichResult, latest: ichResult.pop() ?? null };
  }
  ...
  static async calculateOBV(input: IndicatorInput): Promise<IndicatorResult> {
    ...
    return { indicator: 'OBV', values: obvValues, latest: obvValues[obvValues.length - 1], count: obvValues.length };
  }
  ...
  static async calculateVWAP(input: IndicatorInput): Promise<IndicatorResult> {
    ...
    return { indicator: 'VWAP', sessionType: session_type, values: vwapValues, latest: vwapValues[vwapValues.length - 1], count: vwapValues.length };
  }
